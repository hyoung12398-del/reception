...
const themeSettingKeys = [
  ...
  "quietButtonTextColor",
  "staffButtonBackgroundColor",
  "staffButtonTextColor",
  "staffButtonBorderColor",
  "trialButtonBackgroundColor",
  "trialButtonTextColor",
  "trialButtonBorderColor",
  "rentalButtonBackgroundColor",
  "rentalButtonTextColor",
  "rentalButtonBorderColor",
  "staffCardTextColor",
  "messageColor",
  "baseFontSize",
  "titleFontSize",
  "choiceButtonFontSize",
  "inputFontSize",
];

const colorSettingKeys = themeSettingKeys.filter((key) => key.endsWith("Color"));
...
export async function saveSettings(settings) {
  const nextSettings = {
    ...
    baseFontSize: normalizeFontSize(settings.baseFontSize, defaultSettings.baseFontSize, 14, 24),
    titleFontSize: normalizeFontSize(settings.titleFontSize, defaultSettings.titleFontSize, 24, 52),
    choiceButtonFontSize: normalizeFontSize(settings.choiceButtonFontSize, defaultSettings.choiceButtonFontSize, 16, 34),
    inputFontSize: normalizeFontSize(settings.inputFontSize, defaultSettings.inputFontSize, 16, 34),
  };
...
function fromDeviceRow(row) {
  return {
    ...
    themeOverrides: sanitizeThemeOverrides(row.theme_overrides || {}),
    enabled: row.enabled,
  };
}
...
function toDeviceRow(device) {
  return {
    ...
    theme_overrides: sanitizeThemeOverrides(device.themeOverrides || {}),
...
function fromSettingsRow(row) {
  return {
    ...
    baseFontSize: normalizeFontSize(row.base_font_size, defaultSettings.baseFontSize, 14, 24),
    titleFontSize: normalizeFontSize(row.title_font_size, defaultSettings.titleFontSize, 24, 52),
    choiceButtonFontSize: normalizeFontSize(row.choice_button_font_size, defaultSettings.choiceButtonFontSize, 16, 34),
    inputFontSize: normalizeFontSize(row.input_font_size, defaultSettings.inputFontSize, 16, 34),
  };
}
...
function toSettingsRow(settings) {
  return {
    ...
    choice_button_font_size: settings.choiceButtonFontSize,
    input_font_size: settings.inputFontSize,
...
function sanitizeThemeOverrides(overrides) {
  ...
  for (const key of themeSettingKeys) {
    ...
    if (colorSettingKeys.includes(key)) {
      next[key] = normalizeColor(source[key], defaultSettings[key]);
      continue;
    }

    if (key.endsWith("FontSize")) {
      next[key] = normalizeFontSize(source[key], defaultSettings[key], 12, 56);
      continue;
    }
