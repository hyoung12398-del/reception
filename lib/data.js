...
export async function findTrialRecipients() {
  const settings = await getSettings();
  const staff = await getStaff();
  if (settings.trialLessonStaffIds?.length) {
    return staff
      .filter((item) => settings.trialLessonStaffIds.includes(item.id) && item.enabled && item.slackUserId)
...
export async function saveSettings(settings) {
  const nextSettings = {
    ...
    trialLessonStaffIds: Array.isArray(settings.trialLessonStaffIds)
      ? settings.trialLessonStaffIds.map(String).filter(Boolean)
      : [],
  };
